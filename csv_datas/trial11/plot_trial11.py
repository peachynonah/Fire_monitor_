...
def plot_joint_data(file_number, start_time=None, end_time=None):
    ...
    axs[2, 0].plot(df["current_time"].to_numpy(), df["nonlinear_term_torque_joint1"].to_numpy(), label="Nonlinear Term (J1)", linestyle='--', linewidth = 2.5)
    axs[2, 0].plot(df["current_time"].to_numpy(), df["PD_term_torque_joint1"].to_numpy(), label="PD Term (J1)", linestyle=':', linewidth = 4.5)
    ...
    axs[2, 1].plot(df["current_time"].to_numpy(), df["nonlinear_term_torque_joint2"].to_numpy(), label="Nonlinear Term (J2)", linestyle='--')
    axs[2, 1].plot(df["current_time"].to_numpy(), df["PD_term_torque_joint2"].to_numpy(), label="PD Term (J2)", linestyle=':')
